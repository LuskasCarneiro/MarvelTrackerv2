...
import { packPlaqueCells } from './plaques';
...
describe('packPlaqueCells', () => {
  // Defaults: atlasWidth 2048, cellWidth 512 -> 4 columns, cellHeight 128.
  const COLUMNS = 4;
  ...
  it('places a single cell at the origin', () => {
    ...
    expect(cells[0]).toEqual({ x: 0, y: 0, w: 512, h: 128 });
  ...
  it('rounds height up to the next power of two rather than an exact multiple', () => {
    // 12 plaques / 4 columns = 3 rows * 128px = 384, which is not itself a power of two.
